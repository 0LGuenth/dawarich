...
class Family::MembershipsController < ApplicationController
  ...
  def create
    ...
    if service.call
      redirect_to family_path(@invitation.family),
                  notice: I18n.t('controllers.family.memberships.welcome_to_the_family')
    else
  ...
  def destroy
    ...
    if service.call
      if member_user == current_user
        redirect_to families_path, notice: I18n.t('controllers.family.memberships.you_have_left_the_family')
      else
