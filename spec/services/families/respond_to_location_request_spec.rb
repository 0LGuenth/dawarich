...
RSpec.describe Families::RespondToLocationRequest do
  ...
  describe '#call' do
    it 'accepts a request, enables sharing with the given duration' do
      ...
      expect(target_membership.reload.sharing_active?).to be true
      expect(target_membership.sharing_duration_label).to eq('1h')
    end
    ...
    it 'falls back to the suggested duration on accept' do
      ...
      expect(target_membership.reload.sharing_duration_label).to eq(request.suggested_duration)
    end
    ...
    it 'declines a request without enabling sharing' do
      ...
      expect(target_membership.reload.sharing_active?).to be false
    end
