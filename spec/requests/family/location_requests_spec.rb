...
RSpec.describe 'Family::LocationRequests', type: :request do
  ...
  describe 'POST /families/:family_id/location_requests' do
    before { sign_in owner }

    it 'creates a location request' do
      expect do
        post family_location_requests_path(family), params: { target_user_id: target_user.id }
    ...
    it 'redirects with error when target is already sharing' do
      target_user.membership_for(family).update_sharing!(true, duration: 'permanent')
      post family_location_requests_path(family), params: { target_user_id: target_user.id }
      expect(response).to redirect_to(family_path(family))
    end
  end

  describe 'GET /families/:family_id/location_requests/:id' do
    ...
    context 'when signed in as target user' do
      ...
      it 'shows the request detail page' do
        get family_location_request_path(family, request_record)
  ...
  describe 'GET /families/:family_id/location_requests/:id (non-family member)' do
    let!(:request_record) do
      create(:family_location_request,
             requester: owner, target_user: target_user, family: family)
    end
    ...
    context 'when signed in as a user not in the family' do
    ...
    context 'when signed in as target user requesting non-existent record' do
      ...
      it 'returns 404 for non-existent record' do
        get family_location_request_path(family, id: 999_999)
        expect(response).to have_http_status(:not_found)
      end
    end
  end

  describe 'PATCH /families/:family_id/location_requests/:id/accept' do
    let!(:request_record) do
      create(:family_location_request,
             requester: owner, target_user: target_user, family: family,
    ...
    it 'accepts the request and enables sharing' do
      patch accept_family_location_request_path(family, request_record), params: { duration: '24h' }

      request_record.reload
      expect(request_record).to be_accepted
      expect(request_record.responded_at).to be_present
      expect(target_user.membership_for(family).reload.sharing_active?).to be true
    end
  ...
  describe 'PATCH /families/:family_id/location_requests/:id/decline' do
    let!(:request_record) do
      create(:family_location_request,
             requester: owner, target_user: target_user, family: family,
             status: :pending, expires_at: 1.hour.from_now)
    ...
    it 'declines the request' do
      patch decline_family_location_request_path(family, request_record)
    ...
    it 'does not enable sharing' do
      patch decline_family_location_request_path(family, request_record)
      expect(target_user.membership_for(family).reload.sharing_active?).to be false
    end
  end
end
