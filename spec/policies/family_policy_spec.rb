...
RSpec.describe FamilyPolicy, type: :policy do
  describe 'create?' do
    ...
    context 'when cloud' do
      ...
      # Multi-family: being in a family no longer blocks creating another.
      # The per-user family cap is enforced in Families::Create (MAX_FAMILIES),
      # not the policy, so it surfaces as a proper error rather than "not authorized".
      it 'permits a family-plan user who is already in a family' do
        user = create(:user, plan: :family, skip_auto_trial: true)
